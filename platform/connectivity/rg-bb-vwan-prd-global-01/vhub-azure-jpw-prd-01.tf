...
resource "azurerm_virtual_hub" "vhub_azure_primary_prd_01" {
  name                                   = "vhub-${local.vhub_azure_primary_prd_01.name}"
  resource_group_name                    = azurerm_resource_group.rg_vwan_prd_global_01.name
  location                               = local.vhub_azure_primary_prd_01.location
  virtual_wan_id                         = azurerm_virtual_wan.vwan_prd_global_01.id
  address_prefix                         = local.vhub_azure_primary_prd_01.address_prefix
  branch_to_branch_traffic_enabled       = true
  virtual_router_auto_scale_min_capacity = local.vhub_azure_primary_prd_01.vhub_capacity
...
#   name                = "ergw-${local.vhub_azure_primary_prd_01.name}"
#   resource_group_name = azurerm_resource_group.rg_vwan_prd_global_01.name
#   location            = azurerm_virtual_hub.vhub_azure_primary_prd_01.location
#   allow_non_virtual_wan_traffic = true
#   virtual_hub_id      = azurerm_virtual_hub.vhub_azure_primary_prd_01.id
#   scale_units         = local.vhub_azure_primary_prd_01.ergw_scale_units
# }
...
resource "azurerm_firewall" "afw_azure_jpw_prd_01" {
  name                = "afw-${local.vhub_azure_primary_prd_01.name}"
  location            = azurerm_virtual_hub.vhub_azure_primary_prd_01.location
  resource_group_name = azurerm_resource_group.rg_vwan_prd_global_01.name
  sku_name            = "AZFW_Hub"
  sku_tier            = "Premium"
  ...
  virtual_hub {
    virtual_hub_id  = azurerm_virtual_hub.vhub_azure_primary_prd_01.id
    public_ip_count = local.vhub_azure_primary_prd_01.azfw_public_ip_count
  }
...
resource "azurerm_virtual_hub_routing_intent" "route_intent_azure_jpw_prd_01" {
  name           = "route-intent-${local.vhub_azure_primary_prd_01.name}"
  virtual_hub_id = azurerm_virtual_hub.vhub_azure_primary_prd_01.id
...
resource "azurerm_monitor_diagnostic_setting" "diag_afw_azure_jpw_prd_01_logs" {
  ...
  log_analytics_workspace_id = local.vhub_azure_primary_prd_01.log_analytics_workspace_id
